...
- **YÖK's per-query limit is 2,000 results** — the whole site is searched, but if a query matches more than 2,000 theses only 2,000 are returned and `coverage_complete=false` is reported honestly. **Two ways past it:** (1) `list_university_theses(..., exhaustive=True)` — year-slices to collect ALL of a university's theses completely (live, polite, more requests); (2) the seed index (pre-sliced). **Exception:** topical/keyword "all" search (`islem=4`) cannot be year-sliced (YÖK rejects it), so broad topic queries are subject to the 2,000 cap — narrow the field (title/university) or use the `exhaustive` university listing.
